...
- **Native Single Container Carousel & CSS `sibling-index()` Staggering (PERF-50)**: `MediaPlayerSelector.js` & `media-player-selector.html` embed all media player variants directly as native DOM components (Approach B), completely eliminating `<iframe>` overhead and multi-document memory bloat. All variants subscribe to a **single unified `MediaPlayerCore`** instance and shared `<audio id="audio-player">`, enabling continuous, uninterrupted playback when switching layouts. Layout switches cross-fade smoothly via `document.startViewTransition()`, and sub-components automatically stagger into view using CSS `sibling-index()` (`animation-delay: calc(var(--stagger-step, sibling-index()) * 60ms)`).
